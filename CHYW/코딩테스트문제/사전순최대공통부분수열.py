...
문제 설명:
    ...
    양의 정수로 이루어진 길이가 N인 수열 $\{A_{1}, ...,A_{N}\}$이 주어진다. 
    마찬가지로 양의 정수로 이루어진 길이가 M인 수열 $\{B_{1}...,B_{M}\}$이 주어진다.
...
문제 풀이:
    간단하게 부분 수열을 탐색하려면, 완전 탐색을 해야한다. 즉, 조합이 필요하다.
    하지만 조합으로 풀이 시  시간 복잡도 O(2^n)가 필요하므로 더 단순하게 접근해야한다.

    우리는 공통 부분만이 필요하므로 합집합을 사용하여 공통 부분만 남기면 된다.
    사전순으로 정렬 되어야 하므로 첫 번째 원소가 최댓값이 될 때까지 반복하면 된다.
